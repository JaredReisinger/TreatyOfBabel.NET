...
//    ??         JaredR: Added support for more boolean values.
//    06/09/2010 JaredR: Fixed spelling in several comments.
//               JaredR: Added "Usage:" and indentation to usage text.
//               JaredR: Lots of tweaks to usage text generation.
//    06/15/2010 JaredR: Added "undocumented" argument attribute.
#endregion
//////////////////////////////////////////////////////////////////////////////
...
namespace CommandLine
{
    ...
    public enum ArgumentType
    {
        ...
        Multiple = 0x04,
        /// <summary>
        /// Indicates that the argument is not displayed in usage notes.
        /// </summary>
        Undocumented = 0x08,
    ...
    public sealed class Parser
    {
        ...
        public static bool ParseArgumentsWithUsage(string[] arguments, object destination)
        {
            if (Parser.ParseHelp(arguments) || !Parser.ParseArguments(arguments, destination))
            {
                ...
                Console.WriteLine();
                Console.WriteLine(Parser.ArgumentsUsage(destination.GetType()));
                return false;
            }
        ...
        public static string ArgumentsUsage(Type argumentType, int indentation = 2, int linesBetweenSyntax = 1)
        {
            int screenWidth = Parser.GetConsoleWindowWidth();
            if (screenWidth == 0)
                screenWidth = 80;
            return ArgumentsUsage(argumentType, indentation, linesBetweenSyntax, screenWidth);
        }
        ...
        /// <returns> Printable string containing a user friendly description of command line arguments. </returns>
        public static string ArgumentsUsage(Type argumentType, int indentation, int linesBetweenSyntax, int columns)
        {
            return (new Parser(argumentType, null)).GetUsageString(indentation, linesBetweenSyntax, columns);
        }
        ...
        private const int syntaxHelpTextPadding = 2;
        ...
        private struct ArgumentHelpText
        {
            public ArgumentHelpText(string syntax, string help)
            {
                this.syntax = syntax;
                this.help = help;
            }
        ...
        /// </summary>
        public string GetUsageString(int indentation, int linesBetweenSyntax, int screenWidth)
        {
            List<ArgumentHelpText> allHelpText = GetAllHelpText();
            //ArgumentHelpText[] allHelpText = GetAllHelpText();

            int maxParamLen = 0;
            foreach (ArgumentHelpText helpText in allHelpText)
            {
                maxParamLen = Math.Max(maxParamLen, helpText.syntax.Length);
            }

            // TODO: refactor/rename to make this easier to understand!
            ...
            int helpTextColumn;
            int idealHelpTextColumn = indentation + maxParamLen + syntaxHelpTextPadding;
            screenWidth = Math.Max(screenWidth, minimumScreenWidth);
            if (screenWidth < (idealHelpTextColumn + minimumNumberOfCharsForHelpText))
            {
                helpTextColumn = minimumHelpTextColumn;
            }
            else
            {
                helpTextColumn = idealHelpTextColumn;
            }

            int helpTextCharsPerLine = screenWidth - helpTextColumn;
            StringBuilder builder = new StringBuilder();

            // Get application name, strip path by default...
            string app = Environment.GetCommandLineArgs()[0];
            string appName = Path.GetFileName(app);
            builder.AppendFormat("Usage: {0} [options...]", appName);
            builder.AppendLine();
            builder.AppendLine();

            // TODO: we don't really need the "current column" counter, since there are
            // only a couple of places where we care: after the syntax, at the help text,
            ...
            foreach (ArgumentHelpText helpText in allHelpText)
            {
                if (!fFirst)
                {
                    for (int iLine = 0; iLine < linesBetweenSyntax; ++iLine)
                    {
                        builder.AppendLine();
                    }
                }
                ...
                int syntaxLength = helpText.syntax.Length;
                int helpLength = helpText.help.Length;

                // add syntax string
                builder.Append(' ', indentation);
                builder.Append(helpText.syntax);
                currentColumn += indentation + syntaxLength;
                ...
                while (index < helpLength)
                {
                    ...
                    {
                        endIndex = helpText.help.LastIndexOf(' ', endIndex - 1, Math.Min(endIndex - index, helpTextCharsPerLine));
                        if (endIndex <= index)
                        {
                            // no spaces on this line, append full set of chars
                            endIndex = index + helpTextCharsPerLine;
                        }
                    }

                    // add chars
                    builder.Append(helpText.help, index, endIndex - index);
                    index = endIndex;
                    ...
                    while (index < helpLength && helpText.help[index] == ' ')
                    {
                        index++;
                    }
                }
        ...
        private List<ArgumentHelpText> GetAllHelpText()
        //private ArgumentHelpText[] GetAllHelpText()
        {
            List<ArgumentHelpText> allHelpText = new List<ArgumentHelpText>();
            //ArgumentHelpText[] allHelpText = new ArgumentHelpText[NumberOfParametersToDisplay()];
            ...
            foreach (Argument arg in this.arguments)
            {
                if (!arg.Undocumented)
                {
                    allHelpText.Add(GetHelpText(arg));
                    //allHelpText[index] = GetHelpText(arg);
                    //index++;
                }
            }

            // TODO: make configurable?
            allHelpText.Add(new ArgumentHelpText("@<file>", "Read response file for more options"));
            //allHelpText[index++] = new ArgumentHelpText("@<file>", "Read response file for more options");

            if (this.defaultArgument != null)
            {
                allHelpText.Add(GetHelpText(this.defaultArgument));
                //allHelpText[index++] = GetHelpText(this.defaultArgument);
            }
        ...
        private class Argument
        {
                ...
                if ((stringData != null || type == typeof(bool)) && (stringData == null || stringData.Length > 0))
                {
                    try
                    {
                        ...
                        else if (type == typeof(bool))
                        {
                            ...
                            if (stringData == null ||
                                string.Equals(stringData, "+", ignoreCase) ||
                                string.Equals(stringData, "yes", ignoreCase) ||
                                string.Equals(stringData, "true", ignoreCase) ||
                                string.Equals(stringData, "1", ignoreCase))
                            {
                                value = true;
                                return true;
                            }
                            else if (string.Equals(stringData, "-", ignoreCase) ||
                                string.Equals(stringData, "no", ignoreCase) ||
                                string.Equals(stringData, "false", ignoreCase) ||
                                string.Equals(stringData, "0", ignoreCase))
                            {
                                value = false;
                                return true;
                            }
            ...
            private void AppendValue(StringBuilder builder, object value)
            {
                ...
                {
                    // JaredR: use longer form for default value...
            ...
            public string SyntaxHelp
            {
                get
                {
                    ...
                    else
                    {
                        ...
                        {
                            // JaredR: change help to show more verbose (but more clear)
                            // help string...
                            //builder.Append("[+|-]");
            ...
            public bool Undocumented
            {
                get { return 0 != (this.flags & ArgumentType.Undocumented); }
            }
        ...
        // TODO: convert to type-safe generics?
